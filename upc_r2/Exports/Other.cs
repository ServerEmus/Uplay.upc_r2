...
internal class Other
{
    ...
    {
        Log.Verbose(nameof(UPC_CPUScoreGet), [inContext, outScore]);
    ...
    {
        Log.Verbose(nameof(UPC_GPUScoreGet), [inContext, outScore, outConfidenceLevel]);
        Marshal.WriteInt32(outScore, 0x1000);
    ...
    {
        Log.Verbose(nameof(UPC_ApplicationIdGet), [inContext]);
    ...
    {
        Log.Verbose(nameof(UPC_RichPresenceSet), [inContext, inId, inOptTokenList]);

        var list = Marshal.PtrToStructure<BasicList>(inOptTokenList);
        Log.Verbose(nameof(UPC_RichPresenceSet), [list.ToString()]);
        try
        {
            for (int i = 0; i < list.Count; i++)
            {
                ...
                Log.Verbose(nameof(UPC_RichPresenceSet), [token.ToString()]);
            }
        }
        catch (Exception ex)
        {
            Log.Verbose(nameof(UPC_RichPresenceSet), [ex]);
        }
        return 0;
    }
    ...
    {
        Log.Verbose(nameof(UPC_RichPresenceSet_Extended), [inContext, inId, inOptTokenList, unk1, unk2]);

        var list = Marshal.PtrToStructure<BasicList>(inOptTokenList);
        Log.Verbose(nameof(UPC_RichPresenceSet_Extended), [list.ToString()]);
        try
        {
            for (int i = 0; i < list.Count; i++)
            {
                ...
                var ptr = Marshal.ReadIntPtr(list.List, i * Marshal.SizeOf<UPC_RichPresenceToken>());
                var token = Marshal.PtrToStructure<UPC_RichPresenceToken>(ptr);
                Log.Verbose(nameof(UPC_RichPresenceSet_Extended), [token.ToString()]);
            }
        }
        catch (Exception ex)
        {
            Log.Verbose(nameof(UPC_RichPresenceSet_Extended), [ex]);
        }
        return 0;
    }
    ...
    {
        Log.Verbose(nameof(UPC_LaunchApp), [inContext, inProductId, MustBeZero]);
        return 1;
    }
    ...
    public static IntPtr UPC_ErrorToString(int error)
    {
        ...
        Log.Verbose(nameof(UPC_ErrorToString), [error, switch_ret]);
        return ret;
    }
    ...
    {
        Log.Verbose(nameof(UPC_IsCrossBootAllowed), [inContext, inProductId, outIsCrossBootAllowed, unk1, unk2]);
